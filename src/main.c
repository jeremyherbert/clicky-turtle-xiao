...
void SERCOM0_Handler() {
    if (hri_sercomspi_get_interrupt_SSL_bit(SERCOM0)) {
        ...
        hri_sercomspi_write_DATA_reg(SERCOM0, hid_pending ? 1 : 0);
        pending_index = 0;
    ...
    if (hri_sercomspi_get_interrupt_TXC_bit(SERCOM0)) {
        ...
        if (pending_index == 8 && !hid_pending) {
            hid_pending = true;
        }
    }
}
...
void hid_init() {
    ringbuf_init(&hid_ringbuf, hid_buffer, 256);
}
...
void hid_process() {
    ...
    if (hid_pending) {
        ...
        __disable_irq();
        for (int i=0; i<8; i++) {
            tmp[i] = pending_data[i];
        }
        __enable_irq();
   
        if (tmp[1] == 0x00) {
            ringbuf_push(&hid_ringbuf, tmp, 8);
            hid_pending = false;
        }        
    }
    ...
    pop_len = ringbuf_pop(&hid_ringbuf, data, 8);
...
void usart_init() {
    ...
    hri_sercomusart_write_CTRLA_reg(SERCOM4,
            ...
            SERCOM_USART_CTRLA_SAMPR(2) | // 8x oversampling with arithmetic baud
    ...
    hri_sercomusart_write_BAUD_reg(SERCOM4, 64278); // 115200
